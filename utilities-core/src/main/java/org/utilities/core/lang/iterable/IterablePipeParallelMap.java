...
public class IterablePipeParallelMap<T, R> implements IterablePipe<R> {
	...
	public IterablePipeParallelMap(Iterable<T> it, Function<T, R> mapper, int nThreads) {
		this.it = it;
		this.mapper = mapper;
	...
	private static class It<T, R> implements Iterator<R> {
		...
		public It(Iterator<T> it, Function<T, R> mapper, int nThreads) {
			this.it = it;
			this.mapper = mapper;
			this.nThreads = nThreads;
		...
		private void fill() {
			while (it.hasNext() && notFinishedTasks() < nThreads) {
				T t = it.next();
				Future<R> task = pool.submit(() -> mapper.apply(t));
				tasks.add(task);
			}
		}
